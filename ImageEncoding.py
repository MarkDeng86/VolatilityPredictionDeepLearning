...
class ToImage(object):
    def __init__(self, output_size=(600, 160, 3), bounds="auto", pad = True, include_features=True):
    ...
    def _extract_engineered_features(self, book: pd.DataFrame, trade: pd.DataFrame) -> np.ndarray:
        ...
        features = np.array([
            book_update_count, unique_seconds_count, mean_spread, std_spread, wap1_mean,
    ...
    def __call__(self, sample):
        ...
        # Channel 2 (trade): place size at exact observed second and price bin.
        if not trade.empty:
